...
from inference import XRayInferenceDataset, encode_mask_to_rle
...
# ! Inference Process
def test(models, data_loader, classes, ind2class, thr=0.5, ensemble_thr=2):
    ...
    with torch.no_grad():
        ...
        for step, (images, image_names) in tqdm(
            ...
            ensemble_outputs = np.zeros((len(images), 29, 2048, 2048))
            for model in models:
                images = images.cuda()
            ...
            ensemble_outputs = ensemble_outputs > ensemble_thr
...
def main(args):
    ...
    for i in range(3):  # k = 5
        model = torch.load(os.path.join(args.saved_dir, args.model + f"_fold{i}.pt"))
        models.append(model)
    ...
    infer_transform = A.Resize(1024, 1024)
    ...
    rles, filename_and_class = test(models, test_loader, args.classes, IND2CLASS)
